...
bool screen_saver_on_off = false;
uint64_t screen_saver_timer = 0;
...
void screen_saver(){
  ...
  display.clearDisplay();
  display.drawBitmap(7, 7, bitmap_Array[4], 16, 13, WHITE); // place wallet at X7, Y7
  display.setTextSize(2);
  display.setTextColor(WHITE);
  display.setCursor(28,7);
  display.print("130,401");
  display.fillRect(48, 48, 16, 16, WHITE);
  display.fillRect(64, 48, 16, 16, BLACK);
  display.setTextSize(1);
  display.setTextColor(BLACK);
  display.setCursor(54, 53);
  display.print("L");
  display.setTextColor(WHITE);
  display.setCursor(71, 53);
  display.print("R");
  
  display.display();        // Display the screen
  
  screen_saver_on_off = true;
  screen_saver_timer = 0;
...
void loop() {
  ...
  auto handleButtonPress = [&](int buttonPin, int &key, int &val, int buttonIndex) {
    if (digitalRead(buttonPin) == LOW) {
      ...
      while (digitalRead(buttonPin) == LOW) {
        ...
        if (screen_saver_on_off == true) {
          screen_saver_on_off = false;
        screen_saver_timer = 0;
        main_screen();
        } 
  ...
  // Increment screen_saver_timer
  screen_saver_timer++;
  //Serial.println(screen_saver_timer);

  // Check if screen_saver_timer has reached 42,857,143 which is roughly 5 minutes, 1 minute would be roughly 8,571,429.
  if (screen_saver_timer >= 42857143) {
    screen_saver();
  }
